...
이 디렉터리는 **커맨드 패턴(Command Pattern)** 구현을 위한 C# 코드들을 포함하고 있습니다. 커맨드 패턴은 실행할 명령을 객체로 캡슐화해 명령의 실행, 취소, 재실행 등을 유연하게 관리할 수 있게 해주는 디자인 패턴입니다.
...
- **Concrete Command**  
  실제로 수행할 동작을 정의하며, 각 커맨드별로 해당 동작이 캡슐화됩니다.  
  예를 들어, 오브젝트 이동, 스케일 변환, 회전 등 다양한 명령에 따라 별도의 커맨드 클래스를 정의할 수 있습니다.

- **Command Receiver**  
  커맨드가 실제 동작을 수행할 대상을 의미합니다.  
  예시에서는 Transform 등 실제 명령을 받는 오브젝트가 Recevier 역할을 합니다.
